...
function extractUsernames(segment) {
    const out = [];
...
export function ruleParse(message) {
    ...
    if (/\b(my|show)\b.*\b(position|earning|yield|deposit)\b/i.test(lower) || /\b(what|how much)\b.*(earn|yield|position|morpho)\b/i.test(lower) || /^(my\s+)?positions?\s*$/i.test(lower)) {
        return {
            kind: "earn",
            action: "VIEW_POSITIONS"
        };
    }
    const depositVault = lower.match(/(?:deposit|put|invest)\s+\$?\s*([\d.]+)\s*(?:usdc|usd|dollars?)?\s+(?:into|in|to)\s+(?:vault\s+)?(\d+|morpho|top)/i);
    if (depositVault) {
    ...
    const depositSimple = lower.match(/^(?:deposit|invest|earn\s+with)\s+\$?\s*([\d.]+)\s*(?:usdc|usd|dollars?)?\s*\.?$/i);
    if (depositSimple) {
    ...
    if (/\b(earn|yield|vault|invest|apy|morpho|usdc.*vault|best.*rate)\b/i.test(lower) && !/\b(send|pay|split|register|approve|group)\b/i.test(lower)) {
        return {
            kind: "earn",
    ...
    const register = raw.match(/^(?:i\s+want\s+to\s+)?(?:register|claim)(?:\s+username)?(?:\s+as)?\s+@?([a-z0-9]{3,32})\s*\.?$/i);
    if (register) {
        return {
            kind: "admin",
    ...
    const splitGroupTotal = raw.match(new RegExp(`(?:split|divide)\\s+\\$?\\s*${AMT_TOKEN}\\s+(?:across|in|into)\\s+(?:the\\s+)?(?:group\\s+)?(.+?)\\s*$`, "i"));
    if (splitGroupTotal) {
    ...
    const create = lower.match(/create\s+group\s+['"]?([^'"’]+?)['"]?\s+with\s+(.+)/i);
    if (create) {
    ...
    const casualGroup = raw.match(new RegExp(`(?:i\\s+want\\s+to\\s+)?send\\s+\\$?\\s*${AMT_TOKEN}\\s+to\\s+(?:a\\s+)?group\\s+(.+)`, "i"));
    if (casualGroup) {
    ...
    const casualGroupNamed = raw.match(new RegExp(`(?:i\\s+want\\s+to\\s+)?send\\s+\\$?\\s*${AMT_TOKEN}\\s+to\\s+(?:the\\s+)?(.+?)\\s+group\\b`, "i"));
    if (casualGroupNamed) {
    ...
    const sendGroup = lower.match(new RegExp(`send\\s+\\$?\\s*${AMT_TOKEN}\\s+to\\s+everyone\\s+in\\s+(.+)`));
    if (sendGroup) {
    ...
    const sendSingle = lower.match(new RegExp(`send\\s+\\$?\\s*${AMT_TOKEN}\\s+to\\s+@?(\\w+)`));
    if (sendSingle) {
    ...
    const splitAmong = lower.match(new RegExp(`split\\s+\\$?\\s*${AMT_TOKEN}\\s+among\\s+(.+)`, "i"));
    if (splitAmong) {
