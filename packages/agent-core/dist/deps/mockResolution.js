...
import { createGroup, isWalletRegisteredMock, registerMockUsername, resolveGroup, resolveUsername } from "../resolvers.js";
...
export function createMockResolutionDeps(ctx) {
    return {
        mode: "mock",
        publicClient: null,
        ...
        async resolveGroupByName (name, _wallet) {
            const g = resolveGroup(ctx, name);
            if (!g.ok) {
                return {
                    ok: false,
                    reason: `Unknown group "${name}" (mock).`
                };
            }
            const members = [];
        ...
        async adminCreateGroup (displayName, memberHandles) {
        ...
        async adminRegisterUsername (rawName, wallet) {
            if (!wallet) {
                return {
                    ok: false,
                    reason: "Pass **walletAddress** so we can link your name to your wallet (mock mode)."
                };
            }
...
//# sourceURL=/home/simze/web3-project/SendPay/packages/agent-core/src/deps/mockResolution.ts