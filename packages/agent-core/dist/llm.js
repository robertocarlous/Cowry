...
export async function parseWithLlm(client, userMessage) {
...
export async function generateChatReply(client, userMessage, context) {
    const model = process.env.GROQ_CHAT_MODEL?.trim() || "llama-3.3-70b-versatile";
    ...
    const completion = await client.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: CHAT_SYSTEM + contextNote
            },
            {
                role: "user",
                content: userMessage
