...
import { encodePay, encodePayGroupEqual, encodePayGroupSplit, encodedCallToJson } from "./chain/encodeCowryPay.js";
...
import { clearDraft, getPendingDraft, saveDraft, setPendingDraft, setEarnOpportunities, getEarnOpportunity, setPendingYieldDeposit, getPendingYieldDeposit } from "./state.js";
...
async function buildWelcomeMessage(deps, wallet) {
    const lines = [
        "Welcome to **Cowry**."
    ];
    if (deps.mode === "chain" && wallet && !await deps.isWalletRegistered(wallet)) {
        lines.push("", "**Your wallet does not have a Cowry name yet** — register before sending.");
    }
    lines.push("", "1. Always send **walletAddress** in the API body (the wallet that will sign).", "2. **Register**: **register as yourname** (3–32 chars, a–z and 0–9) — links your @name to your wallet on-chain.", "3. **Pay with USDC or USDm**: e.g. **send 20 USDC to @alice**, **send 5 USDm to @bob**, or **approve 500 USDC for cowry**.", "Say **help** for more.");
    return lines.join("\n");
}
...
export async function paymentFromIntent(intent, deps, wallet) {
    ...
    if (intent.action === "GROUP_SPLIT_TOTAL") {
        ...
        const recipients = g.kind === "onchain" ? g.members.map((addr, i)=>({
                username: shortAddr(addr),
                address: addr,
...
export async function adminFromIntent(intent, deps, wallet, rawText) {
    ...
    if (intent.action === "APPROVE_USDC") {
        ...
        if (deps.mode !== "chain") {
            return {
                kind: "info",
                message: "Mock mode has no USDm/USDC on-chain. With **CELO_RPC_URL** set, **approve 500 usdm for cowry** returns **USDm.approve(CowryPay, …)** calldata."
            };
        }
        if (!wallet) {
            return {
    ...
    if (intent.action === "ADD_MEMBERS") {
        ...
        if (gid == null) {
            return {
                kind: "clarify",
                question: "Say which group id, e.g. **add @mack to group 3** (get ids from **list my groups**)."
            };
        }
        ...
        for (const h of handles){
            ...
            txs.push(encodedCallToJson(encodeAddMember(gid, r.address)));
        }
        return {
            kind: "info",
    ...
    if (intent.action === "REMOVE_MEMBERS") {
        ...
        if (gid == null) {
            return {
                kind: "clarify",
                question: "Say which group id, e.g. **remove @mack from group 3**."
            };
        }
        const handles = intent.members ?? [];
        if (handles.length === 0) {
        ...
        for (const h of handles){
            ...
            txs.push(encodedCallToJson(encodeRemoveMember(gid, r.address)));
        }
        return {
            kind: "info",
...
export async function handleUserMessage(sessionId, message, deps, parseFn, walletAddress) {
    ...
    if (CONFIRM_RE.test(t)) {
        ...
        return {
            ...
            tx: {
                ...
                note: "You sign with your MiniPay wallet (your address pays). Cowry AI builds the transaction; " + "funds move from you via CowryPay, not from the agent wallet.",
                transactions
            },
            ...agent ? {
                agent
    ...
    if (intent.kind === "unknown") {
        ...
        return {
            type: "clarify",
            question: "I didn't quite get that. Try **send $20 to @alice**, **list my groups**, **my balance**, or say **help** for all commands."
        };
    }
    if (intent.kind === "admin") {
